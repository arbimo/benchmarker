...
object TPack extends ISolver {

  override def id: String = "tpack"

  override def accepts(i: DomainVariant): Boolean = i.props("lang") == "tpp"


  // memory limit is enforced by the JVM
   val template = "runsolver -W {TIMEOUT} -v {summary-file} -o {log-file} " +
     "tpack -de -wh 4 -s wastar -we -h ckhadd -o {domain-file} -f {problem-file} -ck {ck-file}"



  override def runner(instance: ProblemId)(implicit p: Params): IO[Exception, Unit] = {
    val props = binders(instance)
    val command = Pattern.bind(template, props.toMap)
    ...
    for {
      ...
      _ <- nio
        .existsAndContains(Path(props("log-file")), "Heuristic")
        .flatMap {
          case true => IO.unit
          case false =>
            IO.fail(Error(s"problem with log file on run of $id / $instance"))
  ...
  override def extract(instance: ProblemId)(implicit p: Params): IO[Exception, RunResult] = {
    val props = binders(instance)
